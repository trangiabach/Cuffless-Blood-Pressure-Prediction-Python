    ...
    for i,(data,label) in enumerate(train_loader):
        ...
        if (i+1) % 1000 == 0:
            ...
            for i,(data,label) in enumerate(test_loader):
                ...
                label_list_sbp.append(label[0])
                output_list_dbp.append(outputs[1])
                label_list_dbp.append(label[1])
                output_list_mbp.append(outputs[2])
                label_list_mbp.append(label[2])

                #print(outputs,label)



            print('Testing SBP MAE in epoch {}: {} '.format(epoch,metrics.mean_absolute_error(label_list_sbp, output_list_sbp)))
            print('Testing DBP MAE in epoch {}: {} '.format(epoch,metrics.mean_absolute_error(label_list_dbp, output_list_dbp)))
            print('Testing MBP MAE in epoch {}: {} '.format(epoch,metrics.mean_absolute_error(label_list_mbp, output_list_mbp)))
